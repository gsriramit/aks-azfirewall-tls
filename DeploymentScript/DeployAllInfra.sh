...
openssl pkcs12 -export -out interCA.pfx -inkey interCA.key -in interCA.crt -password "pass-fw-akstls-02:"
...
az aks create -g $RG_NAME -n $CLUSTER_NAME --enable-aad --enable-azure-rbac --network-plugin azure --node-count 2 --enable-addons monitoring --enable-managed-identity
...
kubectl exec busybox-curl-egresstest -- ls /mnt/secrets-store/

# Copy the certificate file from the mount directory to the CA Certs folder
kubectl exec busybox-curl-egresstest -- /bin/bash

# > cd /mnt/secrets-store
# > sudo mkdir /usr/local/share/ca-certificates/extra
# > openssl x509 -in SelfSignedRootCACert.pem -inform PEM -out SelfSignedRootCACert.crt
# > sudo cp SelfSignedRootCACert.crt /usr/local/share/ca-certificates/extra/SelfSignedRootCACert.crt
